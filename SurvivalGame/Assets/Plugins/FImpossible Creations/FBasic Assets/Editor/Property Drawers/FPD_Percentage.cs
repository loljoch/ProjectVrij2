...
namespace FIMSpace.FEditor
{
    ...
    public class FPD_Percentage : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (editable)
            {
                floatField = new Rect(position.x + position.width - fieldSize + 5, position.y, fieldSize - 18, position.height);
                percField.position = new Vector2(position.x + position.width - 12, percField.position.y);
                percField.width = 18;
            }

            position.width -= fieldSize;
            ...
            {
                if (!editable)
                    EditorGUI.LabelField(percField, Mathf.Round(sliderVal / Attribute.Max * 100f).ToString() + "%");
                else
                {
